...
def extract_individual_features(df, query_text):
    """
    Calcola e aggiunge le feature granulari (una per colonna) al DataFrame.
    """
    print("  -> Calcolando le feature individuali...")
    df_calc = df.copy()
    ...
    print("  -> Feature individuali calcolate.")
    return df_features


def get_general_similarity(df_with_features, query_text):
    """
    Calcola un punteggio di similarità "generale" combinando i campi di testo.
    Questo serve SOLO per il ranking di base.
    """
    print("  -> Calcolando la similarità 'generale' per il ranking di base...")
    df_calc = df_with_features.copy()
    df_calc['system'] = df_calc['system'].apply(lambda x: normalize_loinc_code(x, SYSTEM_MAP))
    df_calc['property'] = df_calc['property'].apply(lambda x: normalize_loinc_code(x, PROPERTY_MAP))

    # Combina il testo come nella tua funzione
...
def main():
    ...
    for sheet in sheets:
        ...
        print(f"Query estratta: '{query_text}'. Trovati {len(df_data)} documenti.")

        # 2. Estrai TUTTE le feature individuali
        df_with_features = extract_individual_features(df_data, query_text)

        # 3. Calcola il ranking di base "ingenuo" (r)
        ...
        print(f"  -> Risultati salvati (con TUTTE le feature) in: '{output_path}'")
